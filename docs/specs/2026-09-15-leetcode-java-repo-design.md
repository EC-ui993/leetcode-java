...
- ❌ 每道题配可运行测试（用户明确不要）——**这不是因为测试不重要，而是因为 LeetCode 本身就是测试套件**：只有 AC 过的代码才允许入库（见 §5.5）。仓库内的编译检查只兜底「代码是否还能编译」（见 §6.3）
...
| 9 | **`--check` 曾发出虚假绿灯** | 早期实现校验完元数据就 `return`，**从未比对索引内容**。于是「加了新题但忘了重建索引」——CI 最该拦住的那个错误——反而拿到 exit 0。已修复：`--check` 先渲染再比对，不一致则退出码 1（§6.2 步骤 7）。**这个缺陷是用户问「CI 是什么」时复盘才发现的**：如果当时直接加了 CI，它会一直绿着，却什么都没守住 |
