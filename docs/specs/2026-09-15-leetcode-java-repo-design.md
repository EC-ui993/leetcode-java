...
| `status` | ✅ | `独立完成` / `看题解完成` / `未通过` | **本仓库的诚信基石**，取值含义见 §5.5 |
| `url` | ✅ | LeetCode 链接 | 替代题面（版权） |
| `date` | ✅ | `YYYY-MM-DD` | **该次 AC 的日期**（不是建档日期——本仓库不存在「已建档但未解出」的中间态，见 §5.5 推论三） |
...
**推论三（不存在「未开始」态）**：本仓库记录的是**已刷过**的题，因此 `status` 里没有 `未开始`。不为「计划要刷的题」预先建目录——那会把仓库变成一份不断变长却几乎不缩短的待办清单，是「每天一道」这类计划最常见的劝退来源。
...
| `--status` | `独立完成`（记录的是刚刷完的题，见 §5.5 推论三） |
| `--slug` | **无缺省，必填** |
...
- 遍历**全部**题目录（无任何跳过规则——`status` 中已不存在「未开始」，见 §5.5 推论三），各自**单独**执行一次 `javac *.java`（见 §5.4 铁律 2）
